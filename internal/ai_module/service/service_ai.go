...
func (s *GrammarService) CheckGrammar(ctx context.Context, sentence string, apiKey option.ClientOption) (*dto.ChatAIResponse, error) {
	client, err := genai.NewClient(ctx, apiKey)
	if err != nil {
		return nil, err
	}
	...
	cs.History = []*genai.Content{
		{
			Parts: []genai.Part{
				genai.Text(`Imagine you are a native English speaker. I want to use you as a tool to have a conversation in English as an exercise for students for speaking material. Please answer all user questions below with the following conditions: The output is a direct answer to the user. Like a two-way conversation, If the user asks a question then you answer according to the context. If the user gives a statement, you respond to the statement.`),
			},
			Role: "user",
		},
	}
